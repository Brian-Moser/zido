...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def __call__(
        ...
        if prompt is not None and isinstance(prompt, str):
            ...
            #        batch_size[i].append(len(prompt[i][j]))
        else:
            batch_size = prompt_embeds.shape[0]
        ...
        if args.local_prompts and h*w>tile_size*tile_size:
            ...
            while cur_y < h:
                ...
                grid_cols += 1
            prompt_embeds_l = []
            ram_encoder_hidden_states_l = []
            for i in range(grid_rows):
                prompt_embeds_l.append([])
                ram_encoder_hidden_states_l.append([])
                for j in range(grid_cols):
                    #print(f"[Tiled Latent]: processing tile {i}x{j}")
                    crop_prompt_embeds, crop_ram_encoder_hidden_states = self._encode_prompt(
                        prompt[i][j],
                        device,
                        num_images_per_prompt,
                        do_classifier_free_guidance,
                        ...
                        ram_encoder_hidden_states=ram_encoder_hidden_states[i][j]
                    )
                    prompt_embeds_l[i].append(crop_prompt_embeds.detach().cpu())
                    ram_encoder_hidden_states_l[i].append(crop_ram_encoder_hidden_states.detach().cpu())
            prompt_embeds = prompt_embeds_l
            ram_encoder_hidden_states = ram_encoder_hidden_states_l
            prompt_embeds_dtype = prompt_embeds[0][0].dtype
        else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                #if i > 0.6*len(timesteps):
                #    tile_size, tile_overlap = (96, 48)
                #    tile_weights = self._gaussian_weights(tile_size, tile_size, 1)
                #el
                if i > 0.5*len(timesteps) or not(args.spot_sr):
                    tile_size, tile_overlap = (64, 32)
                    tile_weights = 1#self._gaussian_weights(tile_size, tile_size, 1)
                else:
                    tile_size, tile_overlap = (64, 0)
                    tile_weights = 1
                #tile_weights = self._gaussian_weights(tile_size, tile_size, 1)
                ...
                if args.local_prompts and h*w>tile_size*tile_size:
                    ...
                    if guess_mode and do_classifier_free_guidance:
                        ...
                        for k in range(grid_rows):
                            ...
                            for j in range(grid_cols):
                                controlnet_prompt_embeds[k].append(prompt_embeds[k][j].chunk(2)[1])
                    else:
                    ...
                    for row in range(grid_rows):
                        ...
                        for col in range(grid_cols):
                            ...
                            if args.local_prompts:
                                #print(controlnet_prompt_embeds[row][col].shape)
                                #print(ram_encoder_hidden_states[row][col].shape)
                                #print(cond_list_t.shape)
                                #if (tile_overlap == 0):
                                #    r = 2*row
                                #    c = 2*col
                                #else:
                                ...
                                down_block_res_samples, mid_block_res_sample = self.controlnet(
                                    cond_list_t,
                                    t,
                                    encoder_hidden_states=controlnet_prompt_embeds[r][c].to(device),
                                    controlnet_cond=img_list_t,
                                    conditioning_scale=conditioning_scale,
                                    guess_mode=guess_mode,
                                    return_dict=False,
                                    image_encoder_hidden_states = ram_encoder_hidden_states[r][c].to(device),
                                )
                            else:
                            ...
                            if args.local_prompts:
                                model_out = self.unet(
                                    input_list_t,
                                    t,
                                    encoder_hidden_states=prompt_embeds[r][c].to(device),
                                    cross_attention_kwargs=cross_attention_kwargs,
                                    down_block_additional_residuals=down_block_res_samples,
                                    mid_block_additional_residual=mid_block_res_sample,
                                    return_dict=False,
